...
@XMLAndDatabaseValues(tableName = "donor", tagName = "donor", defaultVariable = "dnr")
public class Donor extends Loginable
{
	private int id = -1;
	...
	public Donor(int id, int loginableId, String code, String email, String name, String surname, Gender gender, String phoneNumber, LoginableState state)
	{
	...
	public boolean equals(Object comparedObject) 
	{
		if(comparedObject == this)
			return true;
		else if(comparedObject == null || !(comparedObject instanceof Donor))
			return false;
		else
		{
			Donor comparedUser = (Donor) comparedObject;
	...
	public void updateInDB(Connection conn, IDBObject newUser) throws ClassNotFoundException, SQLException
	{
        ...
        try
        {
	        Donor updatingUser = (Donor) newUser;
	...
	//Statik Sorgular
	public static Donor findById(Connection conn, int id) 
			throws ClassNotFoundException, SQLException
	{
		Connection newConn = conn;
		PreparedStatement sqlSt = null;
		...
		try
		{
			...
			TypeDefinition typeDef = new TypeDefinition(Donor.class, "su");
	        ...
	        Donor systemUser = null;
	        
	        if(rs.next())
	        {
	        	systemUser = Donor.parseFromRecord(rs,Loginable.generateColumnNames(typeDef, "l"));
	...
	public static Donor findByStok(Connection conn, String stok) 
			throws ClassNotFoundException, SQLException
	{
		Connection newConn = conn;
		PreparedStatement sqlSt = null;
	...
	//Statik Metodlar
 	public static Donor parseFromRecord(ResultSet rs)
	{
		try
		{
			Donor parsedUser = null;
			int userId = rs.getInt("id");  
			
			if(!rs.wasNull())
			{
				parsedUser = new Donor(userId);
	...
	public static Donor parseFromRecord(ResultSet rs, TypeDefinition typeDefinition, String loginableTableVar)
	{
		try
		{
			...
			if(type != Donor.class || tableVar == null)
	...
	public static Donor parseFromRecord(ResultSet rs, String[] columns)
	{
		try
		{
			Donor parsedUser = null;
