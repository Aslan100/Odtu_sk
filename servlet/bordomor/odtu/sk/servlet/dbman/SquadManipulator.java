...
@WebServlet("/dbman/manipulate_squad.jsp")
public class SquadManipulator extends HttpServlet
{
	private static final long serialVersionUID = 1L;
		
	public SquadManipulator() 
    {
        super();
    }
			...
			throws ServletException, IOException
	{
		...
		try 
        {
			...
			else
			{
				...
				{
					Athlete athleteToRemove = Athlete.findByCode(conn, code);
					processedTeam.removeAthleteInDB(conn, athleteToRemove);
				}
				else if(dataModeStr.equals(Params.DATABASE_INSERT_DATA_MODE))
	        	{
					Athlete athleteToAdd = Athlete.findByCode(conn, code);
					processedTeam.addAthleteInDB(conn, athleteToAdd);
	        	}
	        	else
	        		throw new IllegalArgumentException();
