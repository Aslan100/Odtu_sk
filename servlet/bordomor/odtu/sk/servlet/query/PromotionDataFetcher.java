...
@WebServlet("/query/get_promotion_data.jsp")
public class PromotionDataFetcher extends HttpServlet 
{
	private static final long serialVersionUID = 1L;
	
	public PromotionDataFetcher() 
    {
        super();
    }
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try 
        {
			...
			else
			{
				...
				if(idStr != null)
				{
					Promotion prm = Promotion.findById(conn, Integer.parseInt(idStr));
					
					if(prm != null)
						promotions = new Promotion[] { prm };
				}
				else if(planIdStr != null)
				{
					PaymentPlan plan = PaymentPlan.findById(conn, Integer.parseInt(planIdStr));
					promotions = Promotion.findByBranch(conn, plan.getPaidFor().getPrimaryBranch());
				}
				else
					promotions = Promotion.findAll(conn);
				...
				if(promotions != null)
				{
	        		...
	        		Element promotionContainerEl = xml.createElement("promotions");
	        		
	        		for(Promotion nextPromotion : promotions)
	        			promotionContainerEl.appendChild(nextPromotion.makeXMLElement(xml, true));
	        		
	        		parentEl.appendChild(promotionContainerEl);
