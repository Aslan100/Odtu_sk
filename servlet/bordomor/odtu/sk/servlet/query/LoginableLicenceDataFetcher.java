...
@WebServlet("/query/get_loginable_licence_data.jsp")
public class LoginableLicenceDataFetcher extends HttpServlet 
{
	private static final long serialVersionUID = 1L;
	
	public LoginableLicenceDataFetcher() 
    {
        super();
    }
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try 
        {
			...
			{
				Loginable licenceOwner = Loginable.findByCode(conn, code);
				
				if(licenceOwner != null)
				{
					licenceOwner.fetchDocumentsFromDB(conn);
					
					if(licenceOwner.getDocuments() != null)
					{
						org.w3c.dom.Document xml = XMLUtils.createDocument();
			        	Element parentEl = xml.createElement("query_result");
			        	parentEl.setAttribute("time", new Date().getTime() + "");
		        		xml.appendChild(parentEl);
		        		
		        		Element docsContainerEl = xml.createElement("documents");
	            		
	            		for(Document nextDoc : licenceOwner.getDocuments())
	            			docsContainerEl.appendChild(nextDoc.makeXMLElement(xml, true));
	            		
	            		parentEl.appendChild(docsContainerEl);
