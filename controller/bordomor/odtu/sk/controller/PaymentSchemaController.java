...
@WebServlet(Params.PORTAL_PAYMENT_SCHEMAS_URI)
public class PaymentSchemaController extends HttpServlet 
{
	private static final long serialVersionUID = 1L;

    public PaymentSchemaController() {}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		try
		{
			...
			if(stok != null)
			{
				...
				{
					PaymentSchema[] schemas = loggedUser instanceof ClubManager ? PaymentSchema.findAll(conn) : PaymentSchema.findByBranch(conn, loggedUser.getPrimaryBranch());
					...
					request.getRequestDispatcher("/view/payment_schemas.jsp").forward(request, response);
				}
			}
		}
		catch(Exception ex)
